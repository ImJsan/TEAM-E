...
class tempTrender {
	...
	void tempForMonth(string monthToCalculate) {
		...
		for(int n=0; n<RawDate_.size() + 1; n++){
			if(RawDate_[n].find(monthFilter) != string::npos && RawTime_[n].find(timeFilter) != string::npos){ //Check if the filter criteria is match
	...
	void plotJuneVsMidsummer() {
		tempOnDayInMonth("04","30"); //Extract data for desired month and day
		...
		TH1I* histJune = new TH1I("tempAvg", "Average temperature of June", 50, 0, 20);
		histJune->SetFillColor(kGreen-10);
		histJune->SetLineColor(kBlack);

		//Creating histogram for average temperature of midsummer
		TH1I* histMidsummer = new TH1I("tempAvg", "Average temperature of Midsummer", 50, 1, 31);
	...
	void plotAvergageDayMayToSep() {
		...
		for (int n = 1; n < 32; n++) {
			...
			tempOnDayInMonth("03", helpString);
		...
		for (int n = 1; n < 31; n++) {
			...
			tempOnDayInMonth("04", helpString);
	...
	void plotMidsummerTemp() {
		tempOnDayInMonth("04","30"); //Find temperature for month and day
